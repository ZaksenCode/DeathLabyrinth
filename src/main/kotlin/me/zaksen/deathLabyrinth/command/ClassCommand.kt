...
class ClassCommand(private val config: MainConfig): CommandExecutor {
        ...
        if(sender is Player) {
            ...
            if(playerData?.playerClass == null) {
                if(GameController.getStatus() == GameStatus.PRE_PROCESS) {
                    MenuController.openMenu(sender, ClassChoiceMenu(config))
                } else {
                    sender.message("<red>Сначала начните игру!</red>")
                }
            } else {
                val playerClass = playerData.playerClass!!
                sender.message("<green>Ваш класс: {class_name}</green>", Pair("{class_name}", playerClass.getClassName()))
            }
        }
